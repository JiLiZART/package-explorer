...
export function PackageDashboard({ packageData }: PackageDashboardProps) {
  ...
  const getVersionColor = (version: string) => {
    ...
    if (version.startsWith("5") || version.startsWith("6") || version.startsWith("7")) return "text-purple-400"
    if (version.startsWith("11") || version.startsWith("17")) return "text-yellow-400"
            ...
            <TableBody>
              {outdatedPackages.map((pkg) => (
                <TableRow key={pkg.name}>
                  ...
                  <TableCell className={getVersionColor(pkg.current)}>{pkg.current}</TableCell>
                  <TableCell className={getVersionColor(pkg.wanted)}>{pkg.wanted}</TableCell>
                  <TableCell className={getVersionColor(pkg.latest)}>{pkg.latest}</TableCell>
                  <TableCell>
                    <div className="flex flex-col gap-1">
                      <Badge variant="outline" className={getTypeColor(pkg.type)}>
                        {pkg.type}
                      </Badge>
                      <span className="text-xs text-muted-foreground">Update to 17.1.8 (preserves current range)</span>
                    </div>
                  </TableCell>
